package dev.argon.jvmwasm.format.binary;
...
import dev.argon.jvmwasm.format.types.*;
...
/**
 * A reader for WebAssembly modules.
 */
public class ModuleReader {

	/**
	 * Creates a module reader.
	 * @param is The stream containing the WebAssembly binary format.
	 * @param moduleLength The length of the stream data.
	 */
	...
	private int readS32() throws IOException, ModuleFormatException {
		byte b;
		int value = 0;
		int shift = 0;
	...
	private long readS64() throws IOException, ModuleFormatException {
		byte b;
		long value = 0;
		int shift = 0;
	...
	private String readName() throws IOException, ModuleFormatException {
	...
	private ResultType readResultType() throws IOException, ModuleFormatException {
	...
	private InstrOrTerminator readInstrOrTerminator() throws IOException, ModuleFormatException {
		byte b = readByte();
	...
	private LabelIdx readLabelIdx() throws IOException, ModuleFormatException {
		return new LabelIdx(readU32());
	}

	private FuncIdx readFuncIdx() throws IOException, ModuleFormatException {
		return new FuncIdx(readU32());
	}
	private TypeIdx readTypeIdx() throws IOException, ModuleFormatException {
		return new TypeIdx(readU32());
	}
	private TableIdx readTableIdx() throws IOException, ModuleFormatException {
		return new TableIdx(readU32());
	}
	private LocalIdx readLocalIdx() throws IOException, ModuleFormatException {
	...
	private Start readStartSectionContent() throws IOException, ModuleFormatException {
		var func = readFuncIdx();
	...
	private int readDataCountSectionContent() throws IOException, ModuleFormatException {
		return readU32();
	}
	 ...
	 * @throws IOException when an IO error occurs in the stream.
